...
for i=1:(length(t)-1)
    K1=fun(t(i), u(i,:),mu);
    K2=fun(t(i)+h/2, u(i,:)+h/2.*K1',mu);
    K3=fun(t(i)+h/2, u(i,:)+h/2.*K2',mu);
    K4=fun(t(i)+h, u(i,:)+h*K3',mu);
    u(i+1,:)=u(i,:)+h/3*(K1'/2+K2'+K3'+K4'/2);
end
...
for j=1:length(h)
        ...
        for i=1:(length(t)-1)
            K1=fun(t(i), uh(i,:),mu)';
            K2=fun(t(i)+h(j)./2, uh(i,:)+h(j)./2.*K1,mu)';
            K3=fun(t(i)+h(j)./2, uh(i,:)+h(j)./2.*K2,mu)';
            K4=fun(t(i)+h(j), uh(i,:)+h(j).*K3,mu)';
            uh(i+1,:)=uh(i,:)+h(j)/3*(K1./2+K2+K3+K4./2);
        end
